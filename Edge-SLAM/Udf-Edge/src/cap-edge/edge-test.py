...
class Runner:
    ...
    def gen_extra_pointcloud(self, low_range, indexes, poses, depths, intrisics, intrisics_fs):
		...
		if len(batch_points) >= batch_size_accumulate or gen_nums >= num_points:
		    ...
		    # +++ 关键修改开始：计算剩余所需点数并截取 +++
		    remaining_points_needed = num_points - gen_nums
		    if pc.shape[0] > remaining_points_needed:
		        # 如果当前批的点数超过剩余需求，只取所需部分
		        pc = pc[:remaining_points_needed]
		        if uv is not None: # 如果uv存在，同样截取
		            uv = uv[:remaining_points_needed]
		    # +++ 关键修改结束 +++

		    points_to_add = pc.shape[0] # 实际要添加的点数
		    pc_list.append(pc)
		    uv_list.append(uv)
		    gen_nums += points_to_add # 更新已生成点数
		    batch_points = []  # 清空缓冲区

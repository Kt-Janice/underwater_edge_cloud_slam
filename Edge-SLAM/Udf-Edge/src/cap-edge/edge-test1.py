...
from models.kan_network_gpu import CAPUDFNetwork1
from models.dataset_gpu1 import Dataset
...
class Runner:
    ...
    def run(self):
        ...
        while test:
            ...
            if te :
                ...
                self.load_checkpoint('/home/birlslam/Edge-SLAM/Cloud-Edge-SLAM/Cloud-Edge-SLAM-master/trans_pth/test.pth')
                ...
                if (self.new_udf):
                    ...
                    #print("-------------------------result++++++++++++++++++++:",result)
                    bag = rosbag.Bag('/home/birlslam/Edge-SLAM/Cloud-Edge-SLAM/Cloud-Edge-SLAM-master/trans_bag/test.bag', 'w')
    ...
    def gen_extra_pointcloud(self, low_range, indexes, poses, depths, intrisics, intrisics_fs):
        ...
        i = 0
        # 在循环外预分配缓冲区
        batch_points = []  # 存储多个批次的点云
        batch_size_accumulate = 16  # 累积4个批次再处理
        # 关键修复：直接迭代 DataLoader，不再操作 self.dataset
        for batch in self.train_loader:
            # 正确解包 DataLoader 返回的元组（根据实际结构调整）
            if len(batch) == 2:  # 标准情况：points 和 samples
                points, samples = batch
            else:  # 处理可能返回额外元素的情况
                points = batch[0]
                samples = batch[1]
            ...
            point_std = torch.std(points)
            extra_std = point_std * low_range
            
            rands = torch.normal(0.0, extra_std, size=points.shape, device=device)
            samples = points + rands
            ...
            mask = (udf_sample < 0.01).squeeze(1)
            valid_points = sample_moved[mask]
            
            batch_points.append(valid_points.detach())
            if len(batch_points) >= batch_size_accumulate or gen_nums >= num_points:
                # 批量处理累积的点云
                current_batch = torch.cat(batch_points).cpu().numpy()
                pc, uv = self.construct_covis_graph(
                    current_batch * scale + center,
                    indexes, poses, depths, 
                    intrisics, intrisics_fs, 
                    gen_nums
                )
                pc_list.append(pc)
                uv_list.append(uv)
                ...
                batch_points = []  # 清空缓冲区
            i += 1
            print(f"Generated: {i}/{num_points}")
            if i % 10 == 0:
                torch.cuda.empty_cache()
            # 提前终止条件
            if gen_nums >= num_points:
                break
        return np.concatenate(pc_list), np.concatenate(uv_list)
...
if __name__ == '__main__':
    conf = '/home/birlslam/Edge-SLAM/Udf-Edge/src/cap-edge/confs/base.conf'
    time_1 = time.time()
    runner = Runner(conf)
    runner.run()

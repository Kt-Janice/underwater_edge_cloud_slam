...
public:
    ...
    MergeFinishResult ExitToReplaying(const size_t deferredCount) {
        if (!IsActive()) {
...
class ScopeExit {
public:
    explicit ScopeExit(const std::function<void()> &function)
        : mFunction(function),
          mbActive(true) {
    }
    ...
    void RunAndDismiss() {
        ...
        try {
            ...
            std::cerr << "[CloudMerging][ScopeExit][ERROR] "
                      << exception.what()
                      << std::endl;
        } catch (...) {
            std::cerr << "[CloudMerging][ScopeExit][ERROR] unknown exception"
                      << std::endl;
        }
    }
...
class LocalMapperPauseGuard {
public:
    explicit LocalMapperPauseGuard(LocalMapping *pLocalMapper)
        : mpLocalMapper(pLocalMapper),
          mbReleaseRequired(false) {
    }

    ~LocalMapperPauseGuard() {
        Release();
    }

    LocalMapperPauseGuard(const LocalMapperPauseGuard &) = delete;
    LocalMapperPauseGuard &operator=(const LocalMapperPauseGuard &) = delete;

    bool PauseAndDrain() {
        if (mpLocalMapper == nullptr) {
            throw std::runtime_error(
                "CloudMergeMap requires a LocalMapper when blocking is enabled");
        }

        mbReleaseRequired = true;
    ...
    void Release() {
        if (!mbReleaseRequired || mpLocalMapper == nullptr) {
            return;
        }
...
private:
    ...
    bool mbReleaseRequired;
};

void RelaunchStoppedGlobalBundleAdjustment(
    ...
    CloudMerging *pCloudMerging,
    Map *pActiveMap) {
    if (!bRelaunchRequired) {
        return;
    }
    ...
    if (pCloudMerging == nullptr || pActiveMap == nullptr ||
        pActiveMap->GetOriginKF() == nullptr) {
        std::cerr << "[CloudMerging][GBA][WARNING] stopped GBA cannot be "
                     "relaunched because its map/origin is unavailable"
                  << std::endl;
        return;
    }

    try {
        pThreadGba = new std::thread(
            &CloudMerging::RunGlobalBundleAdjustment,
            pCloudMerging,
            pActiveMap,
            pActiveMap->GetOriginKF()->mnId);
    } catch (const std::exception &exception) {
        pThreadGba = nullptr;
        std::cerr << "[CloudMerging][GBA][ERROR] failed to relaunch GBA: "
                  << exception.what()
                  << std::endl;
    } catch (...) {
        pThreadGba = nullptr;
...
void CloudMerging::Run(const bool bOnline) {
    ...
    if (!bOnline) {
        ...
        CancelPendingCloudMaps(
            CloudMergeOutcome::FAILED_EXCEPTION,
            "offline three-map batching is unsupported by the SLA ticket "
            "dispatcher; use online ticket insertion");
        SetFinish();
        return;
    }
    ...
    while (!CheckFinish()) {
        ...
        try {
            ...
            executionResult.cleanupAction =
                CloudMapCleanupAction::DELETE_CLOUD_MAP;
        } catch (...) {
            executionResult.outcome = CloudMergeOutcome::FAILED_EXCEPTION;
...
CloudMergeExecutionResult CloudMerging::RunSeaCloudMerge(
    ...
    if (!mbWork) {
        result.outcome = CloudMergeOutcome::MERGE_SKIPPED_INVALID_MAP;
        result.detail = "cloud merger work is disabled";
        result.cleanupAction = CloudMapCleanupAction::DELETE_CLOUD_MAP;
        return result;
    }

    if (mpAtlas == nullptr || mpSystem == nullptr) {
        result.outcome = CloudMergeOutcome::FAILED_EXCEPTION;
        result.detail = "sea merge dependencies are unavailable";
        result.cleanupAction = CloudMapCleanupAction::DELETE_CLOUD_MAP;
        return result;
    }

    bool bMapMutationMayHaveOccurred = false;
    try {

    static int nCloudMerge = 0;
    nCloudMerge++;
    const int nLimitMaxMatchKFNum = 50;

    Map *pEdgeFrontMap = mpAtlas->GetSpecifyMap(
        pending.pMap->edgeFrontMapMnId);
    Map *pEdgeBackMap = mpAtlas->GetSpecifyMap(
        pending.pMap->edgeBackMapMnId);
    {
        std::lock_guard<std::mutex> lock(mMutexFinish);
        mpCurrentCloudMap = pending.pMap;
        mpCurrentEdgeFrontMap = pEdgeFrontMap;
        mpCurrentEdgeBackMap = pEdgeBackMap;
    }
            ...
            bool bCloudMapCorrectionApplied = false;
            bool bBackendMergeBlocked = false;
            ...
            auto ensureBackendMergeGuard = [&]() -> bool {
                ...
                const bool bActive = pBackendMergeGuard->IsActive();
                if (!bActive) {
                    bBackendMergeBlocked = true;
                }
                return bActive;
            };
                                        ...
                                        if (kCloudMapCorrectionMode == CloudMapCorrectionMode::SIM3_ONLY) {
                                            ...
                                            if (bSim3Ok) {
                                                ...
                                                if (bApplyOk) {
                                                    ...
                                                    bCloudMapCorrectionApplied = true;
            ...
            if (bSkipCloudMergeDueToCorrectionFailure) {
                if (bBackendMergeBlocked) {
                    result.outcome = CloudMergeOutcome::MERGE_SKIPPED_BACKEND_BLOCK;
                    result.detail = "BackendWriteController rejected sea merge";
                } else if (bCloudMapCorrectionApplied) {
                    result.outcome = CloudMergeOutcome::PARTIAL_FAILURE_AFTER_CORRECTION;
                    result.detail = "sea CloudMap correction was applied before merge failure";
                } else {
                    result.outcome = CloudMergeOutcome::MERGE_SKIPPED_SIM3_FAILURE;
                    result.detail = "selected sea CloudMap correction failed";
                }
            ...
            if (!bSkipCloudMergeDueToCorrectionFailure && (bComputeEdgeFront || mbMergeAnyway)) {
                ...
                ScopeExit gbaRelaunchGuard([&]() {
                    RelaunchStoppedGlobalBundleAdjustment(
                        bRelaunchBA,
                        mMutexGBA,
                        mbRunningGBA,
                        mbFinishedGBA,
                ...
                gbaRelaunchGuard.RunAndDismiss();
                ...
                if (mpNewEdgeFrontEdgeBackKeyFrameMatch.size() < 1) {
                    ...
                    mpCurrentEdgeBackMap->ResetHaveMerged();
                    result.outcome =
                        CloudMergeOutcome::PARTIAL_FAILURE_AFTER_CORRECTION;
                    result.detail =
                        "sea merge failed after edge-front/CloudMap structural "
                        "merge: no edge-back timestamp matches";
                    result.cleanupAction = CloudMapCleanupAction::DELETE_CLOUD_MAP;
                    return result;
                }
            ...
            result.cleanupAction = CloudMapCleanupAction::DELETE_CLOUD_MAP;
            if (!pBackendMergeGuard || !pBackendMergeGuard->IsActive()) {
                result.outcome = CloudMergeOutcome::FAILED_EXCEPTION;
                result.detail = "sea merge completed without an active backend guard";
                return result;
            }

            if (::pSVIn2ORBWrapper == nullptr ||
                !::pSVIn2ORBWrapper->CanReplayDeferredBuffers()) {
                ...
                result.outcome =
                    CloudMergeOutcome::MERGED_REPLAY_ABORTED_SHUTDOWN;
                result.detail =
                    "SVIn2ORBWrapper replay backend is unavailable";
                return result;
            }

            const size_t deferredCount =
                ::pSVIn2ORBWrapper->GetMergeDeferredFrameCount();
            const MergeFinishResult finishResult =
                pBackendMergeGuard->ExitToReplaying(deferredCount);
            if (finishResult == MergeFinishResult::EXITED_TO_IDLE_LOST) {
                result.outcome =
                    CloudMergeOutcome::MERGED_REPLAY_SKIPPED_LOST;
                result.detail =
                    "sea merge completed while LOST backend block was active";
                return result;
            }
            if (finishResult ==
                MergeFinishResult::EXITED_TO_IDLE_SHUTDOWN) {
                result.outcome =
                    CloudMergeOutcome::MERGED_REPLAY_SKIPPED_SHUTDOWN;
            ...
            if (finishResult != MergeFinishResult::ENTERED_REPLAYING) {
                result.outcome = CloudMergeOutcome::FAILED_EXCEPTION;
                result.detail =
                    "BackendWriteController did not enter REPLAYING";
                return result;
            }
            ...
            try {
                ...
                if (mpSystem->GetBackendWriteState() ==
                    BackendWriteState::REPLAYING) {
                ...
                result.outcome = CloudMergeOutcome::FAILED_EXCEPTION;
                result.detail = "unknown deferred replay exception";
                return result;
            }
            if (mpSystem->GetBackendWriteState() ==
                BackendWriteState::REPLAYING) {
            ...
            if (replayOutcome ==
                ...
                result.outcome = CloudMergeOutcome::MERGED_REPLAY_COMPLETED;
                result.detail =
                    "sea merge completed with no deferred replay frames";
                return result;
            }
            if (replayOutcome ==
            ...
            if (replayOutcome ==
                ...
                result.outcome =
                    CloudMergeOutcome::MERGED_REPLAY_ABORTED_WARNING;
            ...
            if (replayOutcome ==
                ...
                result.outcome =
                    CloudMergeOutcome::MERGED_REPLAY_ABORTED_LOST;
        ...
        if (bMapMutationMayHaveOccurred) {
            result.outcome =
                CloudMergeOutcome::PARTIAL_FAILURE_AFTER_CORRECTION;
            result.detail =
                std::string("sea merge exception after map mutation: ") +
                exception.what();
        } else {
            result.outcome = CloudMergeOutcome::FAILED_EXCEPTION;
            result.detail = exception.what();
        ...
        if (bMapMutationMayHaveOccurred) {
            result.outcome =
                CloudMergeOutcome::PARTIAL_FAILURE_AFTER_CORRECTION;
            result.detail =
                "unknown sea merge exception after map mutation";
        } else {
            result.outcome = CloudMergeOutcome::FAILED_EXCEPTION;
            result.detail = "unknown sea merge exception";
        }
        result.cleanupAction = CloudMapCleanupAction::DELETE_CLOUD_MAP;
        return result;
    }
...
CloudMergeTicketPtr CloudMerging::InsertCloudMapWithTicket(Map *pMap) {
    CloudMergeTicketPtr ticket;
    bool rejectInsertion = false;

    {
        std::unique_lock<std::mutex> lock(mMutexCloudQueue);
        ticket =
            std::make_shared<CloudMergeTicket>(++mnNextCloudMergeSequence);
        if (mbSlaShutdownRequested) {
            rejectInsertion = true;
        } else {
            PendingCloudMap pending;
            pending.pMap = pMap;
    ...
    if (rejectInsertion) {
        PendingCloudMap rejected;
        rejected.pMap = pMap;
        rejected.pTicket = ticket;
        ApplyCloudMapCleanup(
            rejected,
            CloudMapCleanupAction::DELETE_CLOUD_MAP);
        CompleteCloudMap(
            ticket,
...
bool CloudMerging::CheckNewCloudMap(const bool bOnline) {
    std::unique_lock<std::mutex> lock(mMutexCloudQueue);
    if (bOnline) {
        return !mlPendingCloudMapQueue.empty();
    }

    return mlPendingCloudMapQueue.size() == 3;
}

void CloudMerging::CompleteCloudMap(
    const CloudMergeTicketPtr &ticket,
    const CloudMergeOutcome outcome,
...
void CloudMerging::CancelPendingCloudMaps(
    ...
    for (PendingCloudMap &pending : pendingCloudMaps) {
        ApplyCloudMapCleanup(
            pending,
            CloudMapCleanupAction::DELETE_CLOUD_MAP);
...
CloudMergeExecutionResult CloudMerging::RunLandAirCloudMerge(
    ...
    if (mpAtlas == nullptr) {
        result.outcome = CloudMergeOutcome::FAILED_EXCEPTION;
        result.detail = "Atlas is unavailable for land/air merge";
        result.cleanupAction = CloudMapCleanupAction::DELETE_CLOUD_MAP;
        return result;
    }
    ...
    nCloudMerge++;
    const int nLimitMaxMatchKFNum = 50;
    const double matchToleranceTime = 0.05;
    ...
    if (mpCurrentEdgeFrontMap == nullptr ||
        mpCurrentEdgeBackMap == nullptr) {
        cerr << "\033[1;33m[Warning] can't find the refer submap! Maps likely culled due to edge-cloud delay. Aborting this merge.\033[0m"
             << endl;
        result.outcome = CloudMergeOutcome::MERGE_SKIPPED_EDGE_MAP_MISSING;
        result.detail = "referenced edge map is unavailable";
        result.cleanupAction = CloudMapCleanupAction::DELETE_CLOUD_MAP;
    ...
    const vector<KeyFrame *> &edgeFrontMapKeyFrames =
        mpCurrentEdgeFrontMap->GetAllKeyFrames();
    const vector<KeyFrame *> &cloudMapKeyFrames =
        mpCurrentCloudMap->GetAllKeyFrames();
    vector<bool> edgeFrontHaveMatchFlag(
        cloudMapKeyFrames.size(),
        false);
    mpEdgeFrontCloudKeyFrameMatch.clear();

    for (unsigned long edgeFrontKF_i = 0;
         edgeFrontKF_i < edgeFrontMapKeyFrames.size();
         edgeFrontKF_i++) {
        if (edgeFrontMapKeyFrames[edgeFrontKF_i] == nullptr) {
            continue;
        }

        double minDelta = 1e9;
        int bestCloudIdx = -1;
        for (unsigned long cloudKF_i = 0;
             cloudKF_i < cloudMapKeyFrames.size();
             cloudKF_i++) {
            if (edgeFrontHaveMatchFlag[cloudKF_i] ||
                cloudMapKeyFrames[cloudKF_i] == nullptr) {
                continue;
            }

            const double deltaTime = std::abs(
                edgeFrontMapKeyFrames[edgeFrontKF_i]->mTimeStamp -
                cloudMapKeyFrames[cloudKF_i]->mTimeStamp);
            if (deltaTime < matchToleranceTime && deltaTime < minDelta) {
                minDelta = deltaTime;
                bestCloudIdx = static_cast<int>(cloudKF_i);
            }
        }

        if (bestCloudIdx != -1) {
            mpEdgeFrontCloudKeyFrameMatch[
                static_cast<int>(edgeFrontKF_i)] = bestCloudIdx;
            edgeFrontHaveMatchFlag[bestCloudIdx] = true;
        }
    }

    if (mpEdgeFrontCloudKeyFrameMatch.empty()) {
        cerr << "\033[1;31m匹配EdgeFront和Cloud Map的KeyFrames数量过少\033[0m"
             << endl;
        result.outcome = CloudMergeOutcome::MERGE_SKIPPED_MATCH_FAILURE;
        result.detail = "no edge-front to CloudMap timestamp matches";
        result.cleanupAction = CloudMapCleanupAction::DELETE_CLOUD_MAP;
    ...
    std::sort(
        vEdgeFrontMatchIndexes.begin(),
        vEdgeFrontMatchIndexes.end(),
        [&edgeFrontMapKeyFrames](const int left, const int right) {
            return edgeFrontMapKeyFrames[left]->mTimeStamp <
                   edgeFrontMapKeyFrames[right]->mTimeStamp;
        });

    const int firstMatchCount = std::min(
        static_cast<int>(mpEdgeFrontCloudKeyFrameMatch.size()),
        nLimitMaxMatchKFNum);
    std::vector<int> vEdgeFrontMatchPartIndexes(
        vEdgeFrontMatchIndexes.begin(),
        vEdgeFrontMatchIndexes.begin() + firstMatchCount);
    std::map<int, int> mEdgeFrontCloudKeyFrameRandSelectMatch;
    for (const int index : vEdgeFrontMatchPartIndexes) {
        mEdgeFrontCloudKeyFrameRandSelectMatch[index] =
            mpEdgeFrontCloudKeyFrameMatch[index];
    }

    cerr << "First Merge EdgeFront Map KF num: "
         << mpCurrentEdgeFrontMap->GetAllKeyFrames().size() << endl;
    cerr << "First Merge Cloud Map KF num: "
         << mpCurrentCloudMap->GetAllKeyFrames().size() << endl;
    cerr << "First Merge Rand Select KF ratio: "
    ...
    const bool bComputeEdgeFront = ComputeSubmapSim3(
        ...
        mpCurrentCloudMap,
        mpEdgeFrontCloudKeyFrameMatch,
        mEdgeFrontCloudKeyFrameRandSelectMatch,
        false,
        mgSwEdgeFrontCloud,
    ...
    ScopeExit firstGbaRelaunchGuard([&]() {
        RelaunchStoppedGlobalBundleAdjustment(
            bRelaunchBA,
            mMutexGBA,
            mbRunningGBA,
    ...
    CloudMergeMap(
        ...
        mbNewUdf);
    mpAtlas->RemoveBadMaps();
    firstGbaRelaunchGuard.RunAndDismiss();

    const vector<KeyFrame *> &newEdgeFrontMapKeyFrames =
        mpCurrentEdgeFrontMap->GetAllKeyFrames();
    const vector<KeyFrame *> &edgeBackMapKeyFrames =
        mpCurrentEdgeBackMap->GetAllKeyFrames();
    std::vector<bool> edgeBackHaveMatchFlag(
        edgeBackMapKeyFrames.size(),
        false);
    mpNewEdgeFrontEdgeBackKeyFrameMatch.clear();

    for (unsigned long newEdgeFrontKF_i = 0;
         newEdgeFrontKF_i < newEdgeFrontMapKeyFrames.size();
         newEdgeFrontKF_i++) {
        if (newEdgeFrontMapKeyFrames[newEdgeFrontKF_i] == nullptr) {
            continue;
        }

        double minDelta = 1e9;
        int bestBackIdx = -1;
        for (unsigned long edgeBackKF_i = 0;
             edgeBackKF_i < edgeBackMapKeyFrames.size();
             edgeBackKF_i++) {
            if (edgeBackHaveMatchFlag[edgeBackKF_i] ||
                edgeBackMapKeyFrames[edgeBackKF_i] == nullptr) {
                continue;
            }

            const double deltaTime = std::abs(
                edgeBackMapKeyFrames[edgeBackKF_i]->mTimeStamp -
                newEdgeFrontMapKeyFrames[newEdgeFrontKF_i]->mTimeStamp);
            if (deltaTime < matchToleranceTime && deltaTime < minDelta) {
                minDelta = deltaTime;
                bestBackIdx = static_cast<int>(edgeBackKF_i);
            }
        }

        if (bestBackIdx != -1) {
            mpNewEdgeFrontEdgeBackKeyFrameMatch[
                static_cast<int>(newEdgeFrontKF_i)] = bestBackIdx;
            edgeBackHaveMatchFlag[bestBackIdx] = true;
        }
    }

    if (mpNewEdgeFrontEdgeBackKeyFrameMatch.empty()) {
        cerr << "\033[1;31m匹配EdgeBack和New Cloud Map的KeyFrames数量过少\033[0m"
             << endl;
        mpCurrentEdgeBackMap->ResetHaveMerged();
        result.outcome =
            CloudMergeOutcome::PARTIAL_FAILURE_AFTER_CORRECTION;
        result.detail =
            "land/air merge failed after edge-front/CloudMap structural "
            "merge: no edge-back timestamp matches";
        result.cleanupAction = CloudMapCleanupAction::DELETE_CLOUD_MAP;
        return result;
    }
    ...
    for (const auto &match : mpNewEdgeFrontEdgeBackKeyFrameMatch) {
        vNewEdgeFrontMatchIndexes.push_back(match.first);
    }
    std::sort(
        vNewEdgeFrontMatchIndexes.begin(),
        vNewEdgeFrontMatchIndexes.end(),
        [&newEdgeFrontMapKeyFrames](const int left, const int right) {
            return newEdgeFrontMapKeyFrames[left]->mTimeStamp <
                   newEdgeFrontMapKeyFrames[right]->mTimeStamp;
        });

    const int secondMatchCount = std::min(
        static_cast<int>(vNewEdgeFrontMatchIndexes.size()),
        nLimitMaxMatchKFNum);
    std::vector<int> vNewEdgeFrontMatchPartIndexes(
        vNewEdgeFrontMatchIndexes.end() - secondMatchCount,
        vNewEdgeFrontMatchIndexes.end());
    std::map<int, int> mNewEdgeFrontEdgeBackKeyFrameRandSelectMatch;
    for (const int index : vNewEdgeFrontMatchPartIndexes) {
        mNewEdgeFrontEdgeBackKeyFrameRandSelectMatch[index] =
            mpNewEdgeFrontEdgeBackKeyFrameMatch[index];
    }

    cerr << "Second Merge Main Map KF num: "
         << mpCurrentEdgeFrontMap->GetAllKeyFrames().size() << endl;
    cerr << "Second Merge EdgeBack Map KF num: "
         << mpCurrentEdgeBackMap->GetAllKeyFrames().size() << endl;
    cerr << "Second Merge Rand Select KF ratio: "
         << mNewEdgeFrontEdgeBackKeyFrameRandSelectMatch.size() << " / "
    ...
    const bool bComputeEdgeBack = ComputeSubmapSim3(
        ...
        mpNewEdgeFrontEdgeBackKeyFrameMatch,
        mNewEdgeFrontEdgeBackKeyFrameRandSelectMatch,
        false,
    ...
    ScopeExit secondGbaRelaunchGuard([&]() {
        RelaunchStoppedGlobalBundleAdjustment(
            bRelaunchBA,
            mMutexGBA,
            mbRunningGBA,
    ...
    mpAtlas->SetMapBad(mpCurrentEdgeBackMap);
    mpAtlas->RemoveBadMaps();
    secondGbaRelaunchGuard.RunAndDismiss();
    ...
    return result;
        ...
        if (bMapMutationMayHaveOccurred) {
            result.outcome =
                CloudMergeOutcome::PARTIAL_FAILURE_AFTER_CORRECTION;
            result.detail =
                std::string("land/air merge exception after map mutation: ") +
                exception.what();
        } else {
            result.outcome = CloudMergeOutcome::FAILED_EXCEPTION;
            result.detail = exception.what();
        ...
        if (bMapMutationMayHaveOccurred) {
            result.outcome =
                CloudMergeOutcome::PARTIAL_FAILURE_AFTER_CORRECTION;
            result.detail =
                "unknown land/air merge exception after map mutation";
        } else {
            result.outcome = CloudMergeOutcome::FAILED_EXCEPTION;
            result.detail = "unknown land/air merge exception";
        }
        result.cleanupAction = CloudMapCleanupAction::DELETE_CLOUD_MAP;
        return result;
    }
...
void CloudMerging::CloudMergeMap(
    ...
    LocalMapperPauseGuard localMapperPauseGuard(pLocalMapper);
    ...
    if (blockLocalMapper) {
        if (!localMapperPauseGuard.PauseAndDrain()) {
            throw std::runtime_error(
                "LocalMapper finished while CloudMergeMap waited for stop");
        }
    }
    ...
    if (vpCurrentMergedMapKFs.size() == 0) {
        ...
        if (blockLocalMapper) {
            if (!localMapperPauseGuard.PauseAndDrain()) {
                throw std::runtime_error(
                    "LocalMapper finished before CloudMergeMap final transfer");
            }
        }
...
void CloudMerging::RequestReset() {
    {
        std::unique_lock<std::mutex> finishLock(mMutexFinish);
        if (mbFinishRequested || mbFinished) {
            return;
        }
        std::unique_lock<std::mutex> resetLock(mMutexReset);
    ...
    while (true) {
        bool resetPending = false;
        {
            ...
            resetPending = mbResetRequested;
        }
        if (!resetPending || CheckFinish() || isFinished()) {
            break;
        }
        usleep(5000);
    }
...
void CloudMerging::RequestResetActiveMap(Map *pMap) {
    ...
    while (true) {
        ...
        {
            std::unique_lock<std::mutex> lock(mMutexReset);
            resetPending = mbResetActiveMapRequested;
        }
        if (!resetPending || CheckFinish() || isFinished()) {
            break;
        }
...
void CloudMerging::ResetIfRequested() {
    ...
    Map *pActiveMapToReset = mpMapToReset;
    if (pActiveMapToReset == nullptr) {
        mbResetActiveMapRequested = false;
        mpMapToReset = nullptr;
        return;
    }

    // mMutexReset remains held until all affected tickets are completed. A
    // RequestResetActiveMap() caller therefore cannot release the active map
    // while its id is read or observe reset completion before queue cleanup.
    const unsigned long activeMapId = pActiveMapToReset->GetId();
    std::list<PendingCloudMap> cancelledCloudMaps;
    {
        std::unique_lock<std::mutex> lock(mMutexCloudQueue);
        auto pending = mlPendingCloudMapQueue.begin();
        while (pending != mlPendingCloudMapQueue.end()) {
            bool isRelatedToActiveMap =
                pending->pMap == pActiveMapToReset;
            if (pending->pMap != nullptr) {
                isRelatedToActiveMap =
                    isRelatedToActiveMap ||
                    pending->pMap->edgeFrontMapMnId == activeMapId ||
                    pending->pMap->edgeBackMapMnId == activeMapId;
            }

            if (isRelatedToActiveMap) {
                auto cancelled = pending;
                pending++;
                cancelledCloudMaps.splice(
                    cancelledCloudMaps.end(),
                    mlPendingCloudMapQueue,
                    cancelled);
            } else {
                pending++;
            }
    ...
    for (PendingCloudMap &pending : cancelledCloudMaps) {
        ApplyCloudMapCleanup(
            pending,
            CloudMapCleanupAction::DELETE_CLOUD_MAP);
        CompleteCloudMap(
...
void CloudMerging::RunGlobalBundleAdjustment(Map *pActiveMap, unsigned long nLoopKF) {
...

    {
        ...
        if (!mbStopGBA) {
            ...
            LocalMapperPauseGuard localMapperPauseGuard(mpLocalMapper);
            if (!localMapperPauseGuard.PauseAndDrain()) {
                mbFinishedGBA = true;
                mbRunningGBA = false;
                return;
...
Map *CloudMerging::GetEdgeFrontMap() {
    std::unique_lock<std::mutex> lock(mMutexFinish);
    // Borrowed snapshot only: callers must not retain it after isRunning()
    // becomes false. Atlas owns the referenced edge map.
    if (!mbRunning) {
        return nullptr;
    }
...
Map *CloudMerging::GetCloudMap() {
    std::unique_lock<std::mutex> lock(mMutexFinish);
    // Borrowed snapshot only: the pending queue remains the sole owner and may
    // delete this map as soon as the current merge finishes.
    if (!mbRunning) {
        return nullptr;
    }

...
// 引入匿名命名空间，确保这两个辅助函数仅在当前文件生效，不污染全局符号表
namespace {
    float ExtractCamParam(ORB_SLAM3::GeometricCamera* pCamera, int index) 
    {
        if (pCamera != nullptr) 
        {
            return pCamera->getParameter(index);
        } 
        else 
        {
            return 0.0f;
    ...
    float ExtractCamInvParam(ORB_SLAM3::GeometricCamera* pCamera, int index) 
    {
        if (pCamera != nullptr) 
        {
            float param = pCamera->getParameter(index);
            if (param != 0.0f) 
            {
                return 1.0f / param;
            } 
...
KeyFrame::KeyFrame() :
    ...
    NLeft(0), NRight(0), mnNumberOfOpt(0), mbHasVelocity(false),mbIsFakeDescriptor(false){
}
...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB) :
    ...
    mvKeysRight(F.mvKeysRight), NLeft(F.Nleft), NRight(F.Nright), mTrl(F.GetRelativePoseTrl()), mnNumberOfOpt(0), mbHasVelocity(false),mbIsFakeDescriptor(false){
    mnId = nNextId++;
...
KeyFrame::KeyFrame(double timeStamp, const Sophus::SE3f &Tcw, const std::vector<cv::KeyPoint> &vKeysUn, 
        const cv::Mat &descriptors, Map* pMap, KeyFrameDatabase* pKFDB, GeometricCamera* pCamera)
    : mnId(nNextId++), mnFrameId(0), mTimeStamp(timeStamp), 
    mnGridCols(FRAME_GRID_COLS), mnGridRows(FRAME_GRID_ROWS),
    mfGridElementWidthInv(0), mfGridElementHeightInv(0),
    mnTrackReferenceForFrame(0), mnFuseTargetForKF(0), mnBALocalForKF(0), mnBAFixedForKF(0),
    mnLoopQuery(0), mnLoopWords(0), mnRelocQuery(0), mnRelocWords(0), mnBAGlobalForKF(0),
    fx(0), fy(0), cx(0), cy(0), invfx(0), invfy(0), mbf(0), mb(0), mThDepth(0), mHalfBaseline(0),
    N(vKeysUn.size()), mvKeys(vKeysUn), mvKeysUn(vKeysUn), 
    mDescriptors(descriptors), // 修正 1：利用 cv::Mat 智能指针浅拷贝，避开 const 检查报错
    mnScaleLevels(1), mfScaleFactor(1.0f), mfLogScaleFactor(0.0f),
    mvScaleFactors(1, 1.0f), mvLevelSigma2(1, 1.0f), mvInvLevelSigma2(1, 1.0f),
    mnMinX(0), mnMinY(0), mnMaxX(0), mnMaxY(0), mK_(Eigen::Matrix3f::Identity()), // 修正 2：补齐四个 const 整型图像边界参数的占位
    NLeft(vKeysUn.size()), NRight(0), mnDataset(0), bImu(false),
    mpCamera(pCamera), mpCamera2(static_cast<GeometricCamera*>(NULL)), 
    mpMap(pMap), mpKeyFrameDB(pKFDB), mpORBvocabulary(static_cast<ORBVocabulary*>(NULL)),
    mbFirstConnection(true), mpParent(static_cast<KeyFrame*>(NULL)), 
    mbNotErase(false), mbToBeErased(false), mbBad(false),
    mbIsFakeDescriptor(true) // 核心：标记伪装骨架状态
{
// 步骤 1：必须预分配 3D 路标点指针数组的内存维度，防止 AddObservation 触发段错误
    mvpMapPoints.resize(N, static_cast<MapPoint*>(NULL));

    // 步骤 2：必须预分配特征点所在网格的容器，防止后续检索指针越界
    mGrid.resize(mnGridCols);
    for (int i = 0; i < mnGridCols; i++) {
    mGrid[i].resize(mnGridRows);
    }

    // 步骤 3：将 SVIn2 传入的高精度先验位姿强行固定至当前关键帧
    SetPose(Tcw);

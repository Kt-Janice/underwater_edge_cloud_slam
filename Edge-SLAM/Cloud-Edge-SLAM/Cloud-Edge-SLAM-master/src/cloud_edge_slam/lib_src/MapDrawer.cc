...
void MapDrawer::newParameterLoader(Settings *settings) { //从yaml文件读取相关参数
            ...
            if (!pKF->GetParent()) // It is the first KF in the map
            {
                ...
                if (bDrawOptLba) {
                    if (sOptKFs.find(pKF->mnId) != sOptKFs.end()) {
                        ...
                        glColor3f(1.0f, 1.0f, 0.0f); // Basic color
                    }
                } else {
                    glColor3f(1.0f, 1.0f, 0.0f); // Basic color //(0.0f, 0.0f, 1.0f)
                }
                glBegin(GL_LINES);
            }

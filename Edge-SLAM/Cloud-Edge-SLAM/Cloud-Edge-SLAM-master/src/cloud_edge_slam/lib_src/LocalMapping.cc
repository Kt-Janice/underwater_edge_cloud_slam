    ...
    while (1) {
        ...
        if (CheckNewKeyFrames() && !mbBadImu) {
            ...
            if (!CheckNewKeyFrames() && !stopRequested()) {
                ...
                // @note 删除多余的关键帧
                if (mbKFCulling) {
                    std::cout << "\033[1;32m[LM-Debug] 9. 准备进入 KeyFrameCulling() 剔除冗余关键帧...\033[0m" << std::endl;
                    CloudKeyFrameCulling();
                    std::cout << "\033[1;32m[LM-Debug] 10. KeyFrameCulling() 平安执行完毕！\033[0m" << std::endl;
                }
...
void LocalMapping::ProcessNewKeyFrame() {
    ...
    if (mpCurrentKeyFrame->mbIsFakeDescriptor == false) {
...
void LocalMapping::CreateNewMapPoints() {
    // [新增防护] 伪装帧没有真实的视觉纹理，绝对不能参与极线搜索与二次三角化
    if (mpCurrentKeyFrame->mbIsFakeDescriptor == true) {
        return;
    }
...
void LocalMapping::SearchInNeighbors() {
    // [新增防护] 伪装帧的拓扑是在注入时硬绑定的，禁止 ORB-SLAM3 执行特征级重复点融合
    if (mpCurrentKeyFrame->mbIsFakeDescriptor == true) {
        return;
    }
...
void LocalMapping::CloudKeyFrameCulling() {
    ...
    for (vector<KeyFrame *>::iterator vit = vpLocalKeyFrames.begin(), vend = vpLocalKeyFrames.end(); vit != vend; vit++) {
        ...
        if (pKF->mbIsFakeDescriptor == true) {
            continue;
        }    
...
void LocalMapping::KeyFrameCulling() {
    ...
    for (vector<KeyFrame *>::iterator vit = vpLocalKeyFrames.begin(), vend = vpLocalKeyFrames.end(); vit != vend; vit++) {
        ...
        // [新增防护] 绝对豁免来自 SVIn2 的伪装骨架帧，禁止剔除，保障刚性拼接拓扑
        if (pKF->mbIsFakeDescriptor == true) {
            continue;
        }

...
CloudImage::CloudImage(cv::Mat img, double timestamp, std::string type) : //调用图像、时间戳、类型
    img(img), timestamp(timestamp), type(type) {
}

CloudImageSampler::CloudImageSampler(System *pSys, int nTrackLast, int nNewTrackFirst, float nTrackLastMinTime, float nNewTrackFirstMinTime, float Kp, float Kd, float th) : //调用frontnum、backnum等参数
    mpSystem(pSys), mState(OLD_MAP_SAMPLE), mTrackingLastState(Tracking::NO_IMAGES_YET), nTrackLast(nTrackLast), nNewTrackFirst(nNewTrackFirst), nTrackLastMinTime(nTrackLastMinTime), nNewTrackFirstMinTime(nNewTrackFirstMinTime) { //ntracklast = frontnum ,nnewtrackfirst = backnum ,ntracklastmintime = frontmintime ,nnewtrackfirstmintime = backmintime
    mpKFDSampler = new KFDSample();
    mpKFDSampler->SetPDKFselectorParams(Kp, Kd, th);//Kp, Kd, th等参数
}

void CloudImageSampler::Reset() { //重置云端图像
    // mEdgeFrontMapId = -1;
    // mEdgeBackMapId = -1;
...
void CloudImageSampler::TrackStep(const int &curTrackingState, const bool &bIsKF, const int &nKFInMap, const int &curMapId, const cv::Mat &img, const double &timestamp) {
    ...
    Map *pEdgeFrontMap = vpMaps[vpMaps.size() - 2]; //选取前图
    Map *pEdgeBackMap = vpMaps[vpMaps.size() - 1]; //选取后图

    if (pEdgeFrontMap->IsHaveMerged() || pEdgeBackMap->IsHaveMerged()) {
        if (pEdgeFrontMap->IsHaveMerged()) { //判断前图是否合并
            //cerr << "Edge Front Map Have Merge !!!!!!!!!!!!!!!!!!!!!!" << endl;
        }
        if (pEdgeBackMap->IsHaveMerged()) { //判断后图是否合并
    ...
    // 当Map KF Num大于20且Duration大于3则进行一次Merge；//当前地图的关键帧数量大于设定的backnum，就进行合并

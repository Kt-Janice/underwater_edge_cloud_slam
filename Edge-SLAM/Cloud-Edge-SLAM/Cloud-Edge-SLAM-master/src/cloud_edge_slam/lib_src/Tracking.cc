...
void Tracking::Track() {
    ...
    if (!pCurrentMap) { //地图集中没有active map
        cout << "ERROR: There is not an active map in the atlas" << endl;
    }

    if (mState != NO_IMAGES_YET) { //NO_IMAGES_YET，还没有接收到输入图像
        if (mLastFrame.mTimeStamp > mCurrentFrame.mTimeStamp) {
            cerr << "ERROR: Frame with a timestamp older than previous frame detected!" << endl; //时间戳乱了
            unique_lock<mutex> lock(mMutexImuQueue);
    ...
    if (mState == NO_IMAGES_YET) {
        mState = NOT_INITIALIZED; //已经接收图像，但是还未初始化
    }
    ...
    if (mState == NOT_INITIALIZED) { //判断状态是否初始化
        ...
        // @note CloudImageSampler Step //将初始化过程的图片送到云端
        ...
        if (!mbOnlyTracking) { //mbOnlyTracking代表仅跟踪模式 //mbOnlyTracking默认为false
            // State OK
            ...
            if (mState == OK) {
                ...
                CheckReplacedInLastFrame(); //更新 local mapping 里的 map points

                if ((!mbVelocity && !pCurrentMap->isImuInitialized()) || mCurrentFrame.mnId < mnLastRelocFrameId + 2) {
                    Verbose::PrintMess("TRACK: Track with respect to the reference KF ", Verbose::VERBOSITY_DEBUG);
                    bOK = TrackReferenceKeyFrame(); //没有帧间速度且imu没有初始化，或者刚完成重定位，与关键帧进行帧间匹配
                } else {
                    Verbose::PrintMess("TRACK: Track with motion model", Verbose::VERBOSITY_DEBUG);
                    bOK = TrackWithMotionModel(); //恒速模型进行匹配
                    if (!bOK)
                        bOK = TrackReferenceKeyFrame(); //恒速失败，继续尝试帧间匹配
                }

                if (!bOK) { //帧间匹配和恒速都没匹配成功
                    if (mCurrentFrame.mnId <= (mnLastRelocFrameId + mnFramesToResetIMU) && (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)) {
                        mState = LOST;
                    } else if (pCurrentMap->KeyFramesInMap() > mpCloudImageSampler->nNewTrackFirst) { //如果当前帧所在的map有很多关键帧，赋值“最近丢失“
                ...
                if (mState == RECENTLY_LOST) {
                    ...
                    if ((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)) {
                        ...
                        if (mCurrentFrame.mTimeStamp - mTimeStampLost > 3.0f && !bOK) { //当前帧丢失过久或者重定位失败，判定为丢失
                            mState = LOST;
                            Verbose::PrintMess("Track Lost...", Verbose::VERBOSITY_NORMAL);
                            bOK = false;
                        }
                    }
                } else if (mState == LOST) { //跟踪失败
                    ...
                    if (pCurrentMap->KeyFramesInMap() <= mpCloudImageSampler->nNewTrackFirst || pCurrentMap->KeyFramesDuration() <= mpCloudImageSampler->nNewTrackFirstMinTime || (pCurrentMap->ComputeTrajCurvature() <= mpCloudImageSampler->nNewTrackFirstMinTrajCurvature)) { //地图关键帧少于backnum或者小于backmintime，reset地图
                        if (pCurrentMap->KeyFramesInMap() <= 5) {                
                            mpCloudImageSampler->TrackStep(LOST, false, pCurrentMap->KeyFramesInMap(), mpAtlas->GetCurrentMap()->GetId(), mImBGR, mCurrentFrame.mTimeStamp);
                            cerr << mpCloudImageSampler->nNewTrackFirst << endl;
                            cerr << "######################################" << endl;
                            mpSystem->ResetActiveMap();
                            //CreateMapInAtlas();
                            return;
                            ...
                            mpCloudImageSampler->nNewTrackFirst = pCurrentMap->KeyFramesInMap();
                            CreateMapInAtlas();
                            return;
                            }
        ...
        if (!mbOnlyTracking) { 
            if (bOK) {
                ...
                ofstream ff;
                ff.open("/home/birl/Udf-Edge/Cloud-Edge-SLAM/Cloud-Edge-SLAM-master/src/cloud_edge_slam/results/Fail_Frame_TimeStamp.txt");
                ff << fixed << setprecision(6)<<mCurrentFrame.mTimeStamp;
                ff.close();
                cout << "Fail to track local map!" << endl;
                cout<<"+++++++++++++++++++++++++++++++++++++++++++++++++++++++++"<<endl;
                cout << "TimeStamp of Fail Frame:"<<fixed <<setprecision(6)<<mCurrentFrame.mTimeStamp<<endl;
                cout<<"+++++++++++++++++++++++++++++++++++++++++++++++++++++++++"<<endl;
               //exit(0); // 0 通常表示程序正常结束
            }
        } else {
        ...
        if (mState == LOST) {
            ...
            if (pCurrentMap->KeyFramesInMap() <= mpCloudImageSampler->nNewTrackFirst || pCurrentMap->KeyFramesDuration() <= mpCloudImageSampler->nNewTrackFirstMinTime || (pCurrentMap->ComputeTrajCurvature() <= mpCloudImageSampler->nNewTrackFirstMinTrajCurvature)) {
                if (pCurrentMap->KeyFramesInMap() <= 5) {         //60        
                    mpCloudImageSampler->TrackStep(LOST, false, pCurrentMap->KeyFramesInMap(), mpAtlas->GetCurrentMap()->GetId(), mImBGR, mCurrentFrame.mTimeStamp);
                    cerr << "######################################" << endl;
                    mpSystem->ResetActiveMap();
                    //CreateMapInAtlas();
...
void Tracking::MonocularInitialization() { //相机初始化，判断初始帧特征点数量是否大于100
    if (!mbReadyToInitializate) {
        ...
        int nmatches = matcher.SearchForInitialization(mInitialFrame, mCurrentFrame, mvbPrevMatched, mvIniMatches, 100); //计算匹配值
...
void Tracking::CreateInitialMapMonocular() { //初始化成功后，使用三角化生成地图点
    // Create KeyFrames
    ...
    mpCloudImageSampler->nNewTrackFirst = 40; // 还原原来的输入值

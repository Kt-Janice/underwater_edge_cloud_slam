...
namespace ORB_SLAM3 {
    ...
    bNeedToUndistort_(false), bNeedToRectify_(false), bNeedToResize1_(false), bNeedToResize2_(false) {
        ...
        //Open settings file
        cv::FileStorage fSettings(configFile, cv::FileStorage::READ);  //再次读取yaml文件
        ...
        //Read first camera
        readCamera1(fSettings); //获取相机类型、内参等数据
        ...
        //Read image info
        readImageInfo(fSettings); //读取图片高、宽、帧率等数据
        ...
        readORB(fSettings); //读取特征提取器的相关参数
        cout << "\t-Loaded ORB settings" << endl;
        readViewer(fSettings); //读取可视化相关参数
        cout << "\t-Loaded viewer settings" << endl;
        readLoadAndSave(fSettings);//没有读取
        cout << "\t-Loaded Atlas settings" << endl;
        readOtherParameters(fSettings);//没有读取
    ...
    void Settings::readCamera1(cv::FileStorage &fSettings) {
        ...
        //Read camera model
        string cameraModel = readParameter<string>(fSettings,"Camera.type",found); //返回相机类型，Pinhole
    ...
    void Settings::readImageInfo(cv::FileStorage &fSettings) {
        ...
        originalImSize_.width = originalCols; //原始宽度，640
        originalImSize_.height = originalRows; //原始高度，480
        ...
        fps_ = readParameter<int>(fSettings,"Camera.fps",found); //帧率24
        bRGB_ = (bool) readParameter<int>(fSettings,"Camera.RGB",found); //true
    }
    ...
    void Settings::readORB(cv::FileStorage &fSettings) {
        ...
        nFeatures_ = readParameter<int>(fSettings,"ORBextractor.nFeatures",found);//所有层特征点数之和，800
        scaleFactor_ = readParameter<float>(fSettings,"ORBextractor.scaleFactor",found);//相邻层缩放系数，1.2
        nLevels_ = readParameter<int>(fSettings,"ORBextractor.nLevels",found);//金字塔层数，8
        initThFAST_ = readParameter<int>(fSettings,"ORBextractor.iniThFAST",found);//检测fast角点阈值，20
        minThFAST_ = readParameter<int>(fSettings,"ORBextractor.minThFAST",found);//没有检测到角点，降低阈值为7
    }

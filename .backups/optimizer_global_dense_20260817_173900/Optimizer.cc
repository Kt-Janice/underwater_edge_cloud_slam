...
void Optimizer::BundleAdjustment(const vector<KeyFrame *> &vpKFs, const vector<MapPoint *> &vpMP,
    ...
    // A monocular initial map contains exactly two keyframes.  Its sparse
    // Cholesky path has been observed to fail while destroying SimplicialLDLT,
    // so use the compatible dense solver for this small initialization BA.
    // General bundle adjustment keeps the existing sparse Eigen solver.
    if (vpKFs.size() == 2U) {
        linearSolver =
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int &num_fixedKF, int &num_OptKF, int &num_MPs, int &num_edges) {
    ...
    list<KeyFrame*> lRealLocalKeyFrames;
    for(list<KeyFrame*>::iterator lit = lLocalKeyFrames.begin(); lit != lLocalKeyFrames.end(); lit++) {
        if(!(*lit)->mbIsFakeDescriptor) {
            lRealLocalKeyFrames.push_back(*lit);
        } else {
            (*lit)->mnBALocalForKF = 0; // 重置标记，免得影响后续图结构
        }
    }
    lLocalKeyFrames = lRealLocalKeyFrames;

    // 如果过滤后发现没有真实的视觉帧，直接跳过整个视觉 BA 优化！
    ...
    for (list<KeyFrame *>::iterator lit = lLocalKeyFrames.begin(), lend = lLocalKeyFrames.end(); lit != lend; lit++) {
        ...
        vSE3->setFixed(pKFi->mnId == pMap->GetInitKFid() || pKFi->mbIsFakeDescriptor);
        optimizer.addVertex(vSE3);
        if (pKFi->mnId > maxKFid)
            maxKFid = pKFi->mnId;
        // DEBUG LBA
    ...
    for (list<MapPoint *>::iterator lit = lLocalMapPoints.begin(), lend = lLocalMapPoints.end(); lit != lend; lit++) {
        ...
        for (map<KeyFrame *, tuple<int, int>>::const_iterator mit = observations.begin(), mend = observations.end(); mit != mend; mit++) {
            KeyFrame *pKFi = mit->first;
            if (pKFi->mbIsFakeDescriptor || pKFi->mpCamera == nullptr) {
                continue; 
            }
            if (!pKFi->isBad() && pKFi->GetMap() == pCurrentMap) {
                const int leftIndex = get<0>(mit->second);
    ...
    if (pbStopFlag)
        if (*pbStopFlag)
            return;
            int fake_kf_count = 0;
            int null_cam_count = 0;
            for(list<KeyFrame*>::iterator lit=lLocalKeyFrames.begin(); lit!=lLocalKeyFrames.end(); lit++) {
                KeyFrame* pKF = *lit;
                if(pKF->mbIsFakeDescriptor) fake_kf_count++;
                if(pKF->mpCamera == nullptr) null_cam_count++;
            }
    optimizer.initializeOptimization();
    ...
    pMap->IncreaseChangeIndex();
    optimizer.clear();  // 这里会释放所有的 g2o 顶点和边
} // 函数结束大括号
